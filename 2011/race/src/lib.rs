...
pub unsafe extern "C" fn best_path(n: c_int, k: c_int, h: *const c_int, l: *const c_int) -> c_int {
    // Safety: n and k must uphold the specification.
    unsafe {
    ...
    // Safety: l must point to an array of length n minus 1.
    let l = unsafe { std::slice::from_raw_parts(l.cast(), n as usize - 1) };

    // Safety: h and l must be the correct size.
    unsafe {
        hint::assert_unchecked(h.len() == n as usize - 1);
        hint::assert_unchecked(l.len() == n as usize - 1);
    }
    ...
    solution::best_path_safe(n as u32, k as u32, h, l)
}
